...
export function TopBar({ title }: TopBarProps) {
    const { isLoaded, currentSnapshot, filters, setFilter, clearFilters } = useDashboard();
            ...
            {isLoaded && (
                <div className="flex items-center space-x-4 relative">
                    ...
                    <button
                        ...
                        <span>Filters</span>
                        {filters.minProbability > 0 && (
                            <span className="ml-1 bg-vjtech-accent text-white text-[10px] px-1.5 py-0.5 rounded-full">{filters.minProbability * 100}%</span>
                        )}
                    </button>

                    {showFilters && (
                        <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-200/60 p-5 z-50">
